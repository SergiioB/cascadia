"""After042 finishes, build/test optional read/compute overlap.
...
STATE = ROOT / 'pipeline-qualification-state.json'
FROZEN = {
        ...
        with (ROOT / 'baseline-queue.lock').open('a+b') as slot:
            ...
            while True:
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError('No completed042 trial within120 minutes')
                time.sleep(15)
            check_frozen()
            candidate = ROOT / 'bin/full-pipeline.exe'
            ...
            manifest = json.loads((ROOT / 'pipeline-source.json').read_text())
            for item in manifest:
                assert hashlib.sha256((ROOT / 'repo' / item['path']).read_bytes()).hexdigest() == item['previous_sha256'], item['path']
            with tarfile.open(ROOT / 'pipeline-source.tar') as archive:
            ...
            with (ROOT / 'test-pipeline.log').open('x', encoding='utf-8') as log:
                process = subprocess.Popen(['cmd.exe', '/c', str(ROOT / 'test-pipeline.bat')],
                                           stdout=log, stderr=subprocess.STDOUT)
                state('building_and_testing', launcher_pid=process.pid)
                try:
                    if process.wait(timeout=3600):
                        raise RuntimeError('Native qualification failed; inspect test-pipeline.log')
                finally:
                    if process.poll() is None:
                        subprocess.run(['taskkill', '/PID', str(process.pid), '/T', '/F'], check=False)
                        process.wait()
            for mode in ('off', 'on', 'uncached'):
                result = json.loads((ROOT / f'fixture-pipeline-{mode}.json').read_text())
                ...
                assert result['pipelined_read_layers'] == (0 if mode == 'off' else 63)
            check_frozen()
            assert hashlib.sha256((ROOT / 'run-full.ps1').read_bytes()).hexdigest() == '6e9dcc5d0d9d053c1239e3f88ffe2dfc943eb1549cde8e8cef710e39002a7c6f', 'Wrapper changed; inspect before replacing'
            (ROOT / 'run-full.ps1').write_bytes((ROOT / 'run-full-pipeline.ps1').read_bytes())
