...
def replay(trace, slots, reset_history, decay_requests=4096):
    states = {}
    ...
    return dict(slots_per_layer=slots, reset_history=reset_history, decay_requests=decay_requests, **totals, cases=cases)
...
def main():
    ...
    result=dict(scope='causal_admission_simulation_not_measured_throughput',source_sha256=hashlib.sha256(raw).hexdigest(),output_hash=trace['output_hash'],results=[replay(trace,slots,reset,decay) for slots in (2,4,8) for reset in (False,True) for decay in a.decay_requests])
