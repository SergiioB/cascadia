"""After046 finishes, build/test optional model-owned routed expert caching.
...
STATE = ROOT / 'expert-cache-qualification-state.json'
FROZEN = {
        ...
        with (ROOT / 'baseline-queue.lock').open('a+b') as slot:
            ...
            while True:
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError('No completed046 trial within120 minutes')
                time.sleep(15)
            check_frozen()
            candidate = ROOT / 'bin/full-expert-cache.exe'
            ...
            manifest = json.loads((ROOT / 'expert-cache-source.json').read_text())
            ...
            with tarfile.open(ROOT / 'expert-cache-source.tar') as archive:
            ...
            with (ROOT / 'test-expert-cache.log').open('x', encoding='utf-8') as log:
                process = subprocess.Popen(['cmd.exe', '/c', str(ROOT / 'test-expert-cache.bat')],
                                           stdout=log, stderr=subprocess.STDOUT)
                state('building_and_testing', launcher_pid=process.pid)
                try:
                    if process.wait(timeout=3600):
                        raise RuntimeError('Native qualification failed; inspect test-expert-cache.log')
                finally:
                    if process.poll() is None:
                        subprocess.run(['taskkill', '/PID', str(process.pid), '/T', '/F'], check=False)
                        process.wait()
            for mode in ('off', 'on', 'uncached', 'pipeline_off'):
                result = json.loads((ROOT / f'fixture-expert-cache-{mode}.json').read_text())
                ...
                assert result['owned_shared_bytes'] == 20736
                assert result['pipelined_read_layers'] == (0 if mode == 'pipeline_off' else 63)
                enabled = mode in ('on', 'uncached')
                cache = result['expert_cache']
                assert (cache['hits'] > 0) == enabled
                assert cache['retained_bytes'] <= cache['capacity_bytes']
                assert cache['capacity_bytes'] == (3 * 1024 * 1024 if enabled else 0)
                for sample in result['samples']:
                    assert sample['generated_ids'] == [28, 48, 106, 84, 28, 48, 106, 84]
            check_frozen()
            assert hashlib.sha256((ROOT / 'run-full.ps1').read_bytes()).hexdigest() == '539662514a8ab1c7ef5549c33f6c31e5c6ae217e3a95e7894b49244fffc21357', 'Wrapper changed; inspect before replacing'
            (ROOT / 'run-full.ps1').write_bytes((ROOT / 'run-full-expert-cache.ps1').read_bytes())
