"""After117/118 exit, qualify bounded runtime prefetch one layer earlier."""
import hashlib,json,os,re,subprocess,tarfile,time
from pathlib import Path
import psutil
ROOT=Path('C:/Users/devcloud/inkling-autolab')
STATE=ROOT/'early-prefetch-qualification-state.json'
...
def qualify(source):
 def unchanged():
  ...
  assert sha(ROOT/'run-full-early-prefetch.ps1')==source['candidate_wrapper_sha256']
 unchanged();candidate=ROOT/'bin/full-early-prefetch.exe';assert not candidate.exists() and not active_full()
 for item in source['files']:assert sha(ROOT/'repo'/item['path'])==item['previous_sha256'],item['path']
 assert sha(ROOT/'early-prefetch-source.tar')==source['archive_sha256']
 with tarfile.open(ROOT/'early-prefetch-source.tar') as archive:
 ...
 log=ROOT/'120-early-prefetch-tests.log'
 with log.open('x',encoding='utf-8') as f:
  proc=subprocess.Popen(['cmd.exe','/c',str(ROOT/'test-early-prefetch.bat')],stdout=f,stderr=subprocess.STDOUT);state('building_and_testing',launcher_pid=proc.pid,launcher_created_unix=psutil.Process(proc.pid).create_time())
 ...
 counts=[int(n) for n in re.findall(r'test result: ok\. (\d+) passed;',log.read_text())];assert len(counts)==13 and sum(counts)==255,counts
 modes=[]
 for name,lead,predict,early in [('off',None,0,0),('current',0,1,0),('early',1,1,1),('previous',1,0,0)]:
  assert not active_full();stem='120-fixture-'+name
  ...
  command=['powershell','-NoProfile','-File',str(ROOT/'run-full-early-prefetch.ps1'),'-Model',str(ROOT/'repo/crates/cascadia-engine-sparse-moe/tests/fixtures/inkling_export'),'-Cases',str(ROOT/'fixture-cases.json'),'-AllowFixture','-Binary',candidate.name,'-Threads','16','-AffinityMask','65535','-Reads','0','-Bf16Rows','2','-Int4Rows','4','-MmapEmbed','1','-ReuseBuffers','1','-SkipBulkPrefetch','1','-OwnShared','1','-UncachedReads','1','-PipelineReads','1','-ExpertCacheMiB','1','-PrefillReads','1','-CacheResetHistory','1','-CacheDecayRequests','32','-CacheRecentTies','1','-PredictReads',str(predict),'-EarlyPredictReads',str(early),'-Tokens','8','-Samples','3','-Out',str(out),'-Log',str(log),'-RouteTrace',str(routes)]
  ...
  assert f'cache_recent_ties=1 predict_reads={predict} prediction_lead_layers={lead or 0} early_predict_reads={early}' in run.stdout
  ...
  assert result['uncached_read_bytes']==0 and result['uncached_read_fallbacks']==(22 if early else old['uncached_read_fallbacks'])
  ...
  modes.append(dict(mode=name,lead_layers=lead,predict_reads=predict,early_predict_reads=early,output_hash=result['output_hash'],expert_cache=result['expert_cache'],prediction_reads=result['prediction_reads'],artifacts=[dict(name=p.name,sha256=sha(p)) for p in artifacts]))
 invalid=command.copy();invalid[invalid.index('-EarlyPredictReads')+1]='1';invalid[invalid.index('-PredictReads')+1]='0'
 rejected=subprocess.run(invalid,text=True,capture_output=True,timeout=30);assert rejected.returncode!=0 and 'Early prefetch requires' in rejected.stdout+rejected.stderr and not active_full()
 unchanged()
 report=dict(status='qualified',source_commit=source['source_commit'],native_tests=sum(counts),binary_sha256=sha(candidate),wrapper_sha256=source['candidate_wrapper_sha256'],modes=modes,full_model_prediction_accuracy_measured=False,full_model_speedup_measured=False,invalid_dependency_rejected=True)
 with (ROOT/'120-early-prefetch-qualification.json').open('x') as f:json.dump(report,f,indent=2)
 (ROOT/'run-full.ps1').write_bytes((ROOT/'run-full-early-prefetch.ps1').read_bytes());assert sha(ROOT/'run-full.ps1')==source['candidate_wrapper_sha256']
 return report

def main():
 import msvcrt
 with (ROOT/'early-prefetch-qualification.lock').open('a+b') as owner:
  owner.seek(0);msvcrt.locking(owner.fileno(),msvcrt.LK_NBLCK,1);assert not STATE.exists()
  try:
   state('waiting_for117_118');deadline=time.monotonic()+3600
   reports=[ROOT/'117-early-prediction.json',ROOT/'118-handle-read-probe.json']
   while not all(p.exists() for p in reports) or active_full():
    if time.monotonic()>deadline:raise TimeoutError('117/118 did not complete')
    time.sleep(15)
   d=json.loads(reports[0].read_text());assert d['correctness_verified'] and d['output_hash']=='ce0fbb9a116d3d09' and d['scope']=='full_large_model_decode' and len(d['samples'])==3
   d=json.loads(reports[1].read_text());assert d['canary_verified'] and d['all_handles_and_buffers_released'] and len(d['pairs'])==30
   with (ROOT/'baseline-queue.lock').open('a+b') as slot:
    slot.seek(0);msvcrt.locking(slot.fileno(),msvcrt.LK_NBLCK,1);assert not active_full();result=qualify(json.loads((ROOT/'early-prefetch-source.json').read_text()))
