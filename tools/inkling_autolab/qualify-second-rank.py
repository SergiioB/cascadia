"""Qualify bounded second-read rank selection without changing default behavior."""
import hashlib,json,os,re,subprocess,tarfile,time
from pathlib import Path
import psutil
ROOT=Path('C:/Users/devcloud/inkling-autolab')
STATE=ROOT/'second-rank-qualification-state.json'
...
def qualify(source):
 def unchanged():
  ...
  assert sha(ROOT/'run-full-second-rank.ps1')==source['candidate_wrapper_sha256']
 unchanged();candidate=ROOT/'bin/full-second-rank.exe';assert not candidate.exists() and not active_full()
 for item in source['files']:assert sha(ROOT/'repo'/item['path'])==item['previous_sha256'],item['path']
 assert sha(ROOT/'second-rank-source.tar')==source['archive_sha256']
 with tarfile.open(ROOT/'second-rank-source.tar') as archive:
 ...
 log=ROOT/'131-second-rank-tests.log'
 with log.open('x',encoding='utf-8') as f:
  proc=subprocess.Popen(['cmd.exe','/c',str(ROOT/'test-second-rank.bat')],stdout=f,stderr=subprocess.STDOUT);state('building_and_testing',launcher_pid=proc.pid,launcher_created_unix=psutil.Process(proc.pid).create_time())
 ...
 counts=[int(n) for n in re.findall(r'test result: ok\. (\d+) passed;',log.read_text())];assert len(counts)==13 and sum(counts)==259,counts
 modes=[]
 for name,lead,predict,early,second in [('off',None,0,0,0),('current',0,1,0,0),('rank1',0,1,0,1),('rank2',0,1,0,1),('rank5',0,1,0,1),('early',1,1,1,0),('previous',1,0,0,0)]:
  rank=int(name[-1]) if name.startswith('rank') else 2
  assert not active_full();stem='131-fixture-'+name
 ...
 invalid=command.copy();invalid[invalid.index('-SecondPredictRank')+1]='1';invalid[invalid.index('-SecondPredictReads')+1]='0'
 ...
 unchanged()
 report=dict(status='qualified',source_commit=source['source_commit'],native_tests=sum(counts),binary_sha256=sha(candidate),wrapper_sha256=source['candidate_wrapper_sha256'],modes=modes,full_model_prediction_accuracy_measured=False,full_model_speedup_measured=False,invalid_dependencies_rejected=4)
 with (ROOT/'131-second-rank-qualification.json').open('x') as f:json.dump(report,f,indent=2)
 (ROOT/'run-full.ps1').write_bytes((ROOT/'run-full-second-rank.ps1').read_bytes());assert sha(ROOT/'run-full.ps1')==source['candidate_wrapper_sha256']
 return report

def main():
 import msvcrt
 with (ROOT/'second-rank-qualification.lock').open('a+b') as owner:
  ...
  try:
   ...
   with (ROOT/'baseline-queue.lock').open('a+b') as slot:
    slot.seek(0);msvcrt.locking(slot.fileno(),msvcrt.LK_NBLCK,1);assert not active_full();result=qualify(json.loads((ROOT/'second-rank-source.json').read_text()))
