...
set CASCADIA_INKLING_PREDICT_READS=0
set CASCADIA_INKLING_EARLY_PREDICT_READS=0
set CASCADIA_INKLING_SECOND_PREDICT_READS=0
set CASCADIA_INKLING_THIRD_PREDICT_READS=0
set CASCADIA_INKLING_SECOND_PREDICT_RANK=2
set CASCADIA_BF16_GEMV_ROWS=2
set CASCADIA_BF16_GEMV_MIN_ROWS=32
...
if exist C:\Users\devcloud\inkling-autolab\bin\full-bf16-grain.exe exit /b 1
...
if errorlevel 1 exit /b %errorlevel%
set CASCADIA_BF16_GEMV_MIN_ROWS=1
cargo +stable-x86_64-pc-windows-msvc test --locked --release -p cascadia-engine-sparse-moe --lib dsv4::math::bf16_tiled_tests::tiled_rows_preserve_production_dot_bits_including_tails
if errorlevel 1 exit /b %errorlevel%
set CASCADIA_BF16_GEMV_MIN_ROWS=16
cargo +stable-x86_64-pc-windows-msvc test --locked --release -p cascadia-engine-sparse-moe --lib dsv4::math::bf16_tiled_tests::tiled_rows_preserve_production_dot_bits_including_tails
if errorlevel 1 exit /b %errorlevel%
set CASCADIA_BF16_GEMV_MIN_ROWS=64
cargo +stable-x86_64-pc-windows-msvc test --locked --release -p cascadia-engine-sparse-moe --lib dsv4::math::bf16_tiled_tests::tiled_rows_preserve_production_dot_bits_including_tails
if errorlevel 1 exit /b %errorlevel%
cargo +stable-x86_64-pc-windows-msvc build --locked --release -p cascadia-engine-sparse-moe --example inkling_decode_bench
if errorlevel 1 exit /b %errorlevel%
if exist C:\Users\devcloud\inkling-autolab\bin\full-bf16-grain.exe exit /b 1
copy C:\Users\devcloud\inkling-autolab\target\release\examples\inkling_decode_bench.exe C:\Users\devcloud\inkling-autolab\bin\full-bf16-grain.exe
if errorlevel 1 exit /b %errorlevel%
exit /b 0
