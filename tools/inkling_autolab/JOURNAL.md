...
085 diagnostic hypothesis: join causal replayed per-layer cache misses with074 actual per-layer timings to distinguish storage-sensitive visits from all-cached compute visits. Replay aggregate hits/misses/admissions/evictions must match074 exactly before accepting the join. Group measured MLP time by0–6 misses; report correlations only, not a causal speedup or a25tok/s prediction. No runtime changes or PTL load.

084 staged/SHA checked and waiting, nativePython3204,parent10144, no component reads until080/full exit.085 joins all36288 sparse-layer visits and matches every aggregate cache counter. MedianMLP with0–6misses3.795/5.373/8.540/11.572/14.578/17.582/20.617ms; correlation only.077 rows1/4 finishes0.9273579552 versus0762/4control0.9244748903, a small difference below074 record; no promotion yet.078 rows4/4 active6304 created1789375493.4175692,079/080queued.

086 storage-layout hypothesis: file fragmentation may limit whole-expert read throughput and could confound comparisons to newly written compressed fixtures. Prepare a read-only FSCTL_GET_RETRIEVAL_POINTERS sample of36 original experts across six layers after084. Query allocation extents and cluster geometry only; no data reads, defragmentation, file moves, volume changes or model writes. Source docs: https://learn.microsoft.com/en-us/windows/win32/api/winioctl/ni-winioctl-fsctl_get_retrieval_pointers and https://learn.microsoft.com/en-us/windows/win32/api/winioctl/ns-winioctl-retrieval_pointers_buffer .

078 rows4/4 complete0.9311444128, all exact gates pass, raw artifacts captured.079 failed during SSH banner exchange before command launch, with no raw output; original failed campaign/history retained. Host recheck confirmed no active full process and no079 native files. Prepared079b identical retry using original079 native output paths and30sConnectTimeout;080 gets same transport timeout. Resume controller will run079b then080;084 waiter still correctly watches native079/080 completion. No benchmark or correctness failures were bypassed.

087 topology hypothesis: prior worker counts varied scheduling across all16 processors; they did not isolate CPU efficiency classes. Query read-only GetSystemCpuSetInformation and join logical processor classes with saved074 decode-only utilization before considering any process-affinity experiment. No machine power setting or affinity changes during this observation. API sources: https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-getsystemcpusetinformation and https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-system_cpu_set_information .

087 read-only topology reports higher efficiency-class1 onCPU0–3;CPU4–15 class0. CPU0–11 share last-level-cache index0;CPU12–15 share separate index12. During074 decode, all16 have material machine-wide utilization (~44–61%). This does not directly isolate model utilization or prove affinity will help.

088–091 hypothesis: restricting the benchmark process to the0–11 shared-cache domain may reduce migration/cache-transfer overhead; compare12 and16 worker pools against fresh16workers/all16cores. Stage a wrapper with optional AffinityMask(default65535 unchanged), read actual child affinity back and gate it. Native tiny qualification of65535/16,4095/12,4095/16 after084+086, then full campaigns. No global power/affinity settings, frozen binary unchanged. Keep selectedBF16rows2/int4rows4/cache256/decay32 profile during this comparison unless row results justify a separately confirmed change first.

080 rows2/2 complete0.9352964152, all hashes/routes/counters match076–079. Rowcomparison080:2/4control.924475,1/4.927358,4/4.931144,2/1.929776,2/2.935296.2/2 improves every prompt by0.75–1.17% against freshcontrol; retain as candidate for repeatedconfirmation. Confirmed profile remains2/4 at.938274; upcoming affinity isolates placement with that profile. Raw080 artifacts/SHA/profilecaptured. Controller66059 exited normally.084 now probing,0863604 waits,088native10528(parent3964) waits for both. Newwrapper staged/SHAchecked, native production wrapper stillunchanged until088qualified.

084 completed30matchedpairs/120experts withallSHA/errorcanariespass; all30compressedarmslose. For2/4/6files, originalmedian11.310/19.371/25.215ms versuscompressed+decode40.443/47.113/48.949ms. Decode-only26.51–26.89ms. Reject runtimeZstd:13% bytesavings do not offset CPUdecoder cost. Alltempfixturesremovedandjob3204exited.086metadataquerycomplete:36/36 originalfiles spanmultiplephysicalruns, median5,max6. It did not change any file.088 affinityqualified3modes, wrapperpromotedandSHAverified;10528/3604exited. All084/086/088 artifactsbundled/copied/SHAverified.089–091 controller43106 running, script/private/tmp/run-inkling-affinity-loop.py.

092 hypothesis: originalfiles have4–6 physicalruns; temporary sequential byte-identical copies might yield fewer runs and lower uncached read latency without compression. Prepare30matchedoriginal/copycohorts2/4/6files, balancedAB/BA, record extents forboth, hash eacharm outside timing. Create/copy only task-owned temporaryfiles; neverreplace ormodify modelpaths. Removealltempfixturesafterprobe. Wait for089–091/full exit before takingqueue lock. This tests layout; any future model-file relocation requires separate review of concrete byte-verification/rollback implementation.
...
089–091 complete: all16/.926826 beats mask4095 with12workers/.889149 or16workers/.890635. All numerical and actual counter gates passed; retainall16. Raw artifacts and checks archived.092 completed30matched read pairs, median physical runs5original/1copy. Median paired gains for2/4/6files .831/1.058/1.052; copy wins4/10,7/10,10/10.2file order effects are large. Do not claim full-model gain or move the model from this evidence. All120fileSHA checks pass, temporary copies removed, native process exited.

096 hypothesis: current ceil-halving leaves old single observations atfrequency1, and strict-frequency admission can protect stale entries on ties. Causally replay floor/ceil decay and strict/recent tie rules for intervals8/16/32/64/128 with8slots perlayer. Observe current cohorts before gate-order admission, retain entries across requests, reset history. First require the unmodified32/ceil/strict control to reproduce all074 counters and case-level prior replay. This is offline read-count prediction, not throughput, and can run locally during094/095. Runtime changes depend on the evidence.

096 result:32/ceil with recency admission on equal frequency predicts117875misses versus123493control (4.549% fewer reads), with4.34–4.85% reduction in every sample. Admissions rise25801→80366; allocations are exchanged rather than copied, but full timing must include churn. Floor rounding offers no advantage. Implement only opt-in CASCADIA_INKLING_CACHE_RECENT_TIES, keeping default strict policy and32/ceil unchanged. Compare complete (frequency,last-use) keys only when enabled; preserve immutable leases, gate-order admission, byte budgets, and output math. Add actual recent-tie admission count, stale-tie/within-cohort ordering and leased-victim/real-byte tests. Native build and oracle qualification wait until094/095 finish.

097 local validation:241 tests across13suites passed, including three newpolicy tests; benchmark example builds. Source849a08bd pushed with t8 author/committer, no coauthor.098 staging matches allSHA; native production wrapper and source files match expected baseline before launch; candidate PS parser passed.098 detached parent10896 waits for both094/095 reports and full-process exit. It will preserve all9frozen binaries and replace production wrapper only after245expected native tests plusfive tiny wrapper modes.

099 read-only memory ownership follow-up: existing Qwen3.6-35B-A3B OVMS PID6728 has20.58GB private commit and20.03GB sharedGPU allocations, while its ordinary working set is only4MB under pressure. GPU accounting explains the otherwise missing physical headroom. This confirms the historical protected-service constraint; no process/service/model unload attempted. Preserve OVMS/node/CA.

100/101 full recency campaigns prepared with same binary and exact predicted counters; do not launch until098 qualifies. Currently rows2/4 in both; review094/095 before freezing the final choice. The recency candidate predicts39319misses/1252329652224uncached bytes versus41193/1312017481728control.Larger future cache capacity experiments require actual memory headroom; no cap increase here.

103 research hypothesis, pending implementation: predict a layer's upcoming experts from its current residual input before attention, using the existing MLP norm/router; run actual routing after attention unchanged. Diagnostic-only first: capture predicted and actual selections, with no reads or cache-history mutation, then causally score uncached prediction precision and read amplification against traces. This may expose an attention/read overlap opportunity that previous-token route prediction missed. Existing published work supports investigating pre-attention/cross-layer router predictors, but its measured accuracy/speedups are not Inkling/PTL evidence and trained/changed routers are outside this experiment. Primary sources: https://arxiv.org/abs/2502.12224, https://arxiv.org/abs/2511.10676, https://arxiv.org/abs/2308.12066. Do not modify the staged098 source/binary while it is awaiting qualification. Future diagnostic patch should be staged only after098, with its own oracle.

103 diagnostic implementation local: separate decode-only pre-attention route observer evaluates existing MLP norm/router on the current residual input. Actual route observer remains unmodified in meaning; predicted routes do not notify it or affect cache history. Benchmark --prediction-trace requires an actual --route-trace and distinct nonexisting paths. All243 local tests pass, including exact-logit/actual-route and different-attention-state causality checks. Three-repeat ARMfixture hash5122e042f9b1fb30, all actual route arrays and cache counters match unobserved control. Analyzer reproduces actual cache counters, rejects missing predictions/wrong hashes/changed actual routing, and selects predicted uncached experts before scoring. No prefetch I/O or full-model prediction accuracy measured. Future native deployment must occur after098; source staged for098 remains849a08bd unchanged.

094/095 repeated row comparison complete:2/4control0.9354823511 versus2/2candidate0.9355367858, just0.00582% conservative-score gain. Exact outputs/hash/actual counters pass. The earlier1.17% single-pass gain did not reproduce; retain2/4. No newrecord or profile promotion. Native full processes5132/7928 and controller95987 exited.098qualification may now build the frozen849a08bd candidate; controller70713 will archive its tests and run100/101 sequentially.

098 complete:245 native tests andfive tiny wrapper modes passed; all9 olderbinaries unchanged. Newfull-cache-recency.exe SHAa23289477c2d5ade1e838ccf92d27ec8b5d31b1cebc4d79bc7d74ec85d1daa40/source849a08bd. Wrapper promoted, SHA2217f5076314e4462bc95dbe2e5d6a643981e65d11a2444bd412813d59ed1f97. Python3144/cmd2616 exited; all artifactsSHAcopied. Controller70713 started100control native9956(created1789379922.80522),101recent queued.102repeatedconfirmation prepared conditionally, notlaunched.103 remainslocal-only source67973647.

104 native prediction qualification prepared after100/101: isolate a newfull-route-prediction.exe, preserve all10older binaries, test247native tests and four tiny modes (predictionoff/on × recenttiesoff/on), require exact outputs/actual route arrays/cache counters. No speculative I/O. It may build after101 before conditional102confirmation;102must wait104qualification exit. Keep current native/localrun-full.ps1 stable while100/101 run; stage a distinctrun-full-prediction.ps1, promote only afterqualification. Then102 can confirm the unchanged frozenrecencybinary, and105future fullpredictiondiagnostic can measure forecast quality.

101 completed with exact outputs/routes/cache counters. Actual decode reads fell4.549%; binary search and short story each gained about4%. Water-cycle second token took6.793s in summed layers versus1.576s control, with stalls inMLP layers43/51/59. The stall remains included; no record claimed and no established cause. Conservative scores100=.9281253880,101=.9282463125. Run102 nine-sample confirmation to test gains and recurrence before promotion.

104 qualified247 native tests and four tiny modes, unchanged actual routes/logits/cache counters and all10 older binaries. Source32e2de69; full-route-prediction.exe SHA89c0f361ac53bbae6af52979d889216138800296c99699976dbeb1732da93984. Raw artifacts SHA archived. Native/local production wrapper now both b0af52ea911f1a46ffc14f2a0f3588689443987a17db8aa1d60a0cea84d0934c.102 started native8204(created1789380877.6939435), controller94756.105 diagnostic prepared pending102 policy decision. Controller now prevents diagnostic_only/promotion_allowed=false campaigns from satisfying the25tok/s target;15 controller/resource tests pass. No actual prefetch/export/Lambda work.

106/107 planned validation hypothesis: the recent-tie cache and causal pre-attention predictions should transfer to unseen tasks and128-token continuations. Freeze worked algebra, JSON inventory and engineering dialogue prompts now inheldout-prompts.json, before seeing their routes. After105, generate a strict-policy reference using the qualified frozen runtime; the recent-policy diagnostic must match every ID and full-logits hash. These runs are explicitly ineligible for the established performance record. No tuning on held-out routes before initial comparison.

102 verified allnine samples: slowest.9654120906, median.9916481036, fastest1.0604665236. Allsamples improve2.15–3.20% vs074; conservative gain2.8923%,7.1336×original. Fullhash/IDs/routes/cachecounters andSHA artifacts match. Actualreads3.754TB, down4.549%. NoMLPevent>.5s;101 second-token stall doesnot recur (102firstwatersecondtoken1.582s). Firstsamples/stallsremainincluded. MinavailableRAM1.70GB, peakprivate42.51GB. Promote recent-tie policy;25targetstillunmet.105diagnostic nowcontroller34745; no actualprefetch.

105 full prediction diagnostic completes with exact outputs/routes/cache counters. Choosing first uncached prediction yields95.229% precision,28.836% actual miss coverage,1.4446% extra reads (11338 useful/568 unused). Choosing two drops precision90.41% and adds5.57% reads. No actual prefetch occurred.106 unseen-prompt reference launch requested after105 capture; runtime policy choice is frozen to one predicted expert before held-out traces are viewed.

108 runtime hypothesis: start one predicted uncached expert read on a dedicated bounded background reader before attention, consume its complete bytes only if actual routing selects that expert, and drain unused/failed reads before the layer returns. Actual routing, cache history/admission order and kernels remain unchanged. Preserve complete normal-read fallback on prediction failure. A single pending expert per layer limits added memory; actual useful/unused/failed-read counters and byte accounting must match causal replay. Qualify native exact output tests before full A/B trials, then confirm any repeated gain. This is a new runtime experiment, not a claimed105 speedup.

108 implementation uses one bounded std::sync::mpsc::sync_channel worker independent of Rayon, with owned response buffers and draining RAII. Thread-spawn/disconnect failures fall back to normal reads. Primary API sources checked: https://doc.rust-lang.org/std/sync/mpsc/fn.sync_channel.html and https://doc.rust-lang.org/std/thread/struct.Builder.html .27 focused Inkling unit tests pass, including five new membership/worker/ownership tests. Full248-test local suite and example build pending.106 native596(created1789382341.9607809), Python3092, launcher5324 active.

108 local qualification complete:248 tests/13suites, four tiny complete-model modes (cacheoff/on × predictedreadoff/on), all outputs/hash/actual routes/cache counters exact. Enabled tiny path schedules13 reads,11useful/2unused, zero failures; cache0 schedulesnone. Causal replay of native104 fixtures independently predicts13/11/2. Benchmark prediction traces now truthfully mark prefetch_performed when reads occur. Frozen native106/107 sources remain unchanged; new candidate wrapper/run-full-prefetch.ps1 and test-predicted-read.bat prepared, not deployed.

106 complete andSHA captured, native596/Python3092 exited. Unseen referencehash e396cc533e658e44, three128-token prefixes reviewed coherent/responsive (truncatedJSON is not a complete validatedJSONdocument). Initialreference is not correctness-verified or record eligible. Strict counters87070misses/59234hits; frozenrecentpolicy replay predicts82859misses/63445hits,4.836% fewerreads,58482admit/57970evict/45872ties.107 will verify all128IDs/hash and those counters while recording pre-attention predictions; timing includes added diagnostic overhead and is not a pure cache A/B.

109 native qualification prepared: preserve11 frozen binaries, verify source replacements, require252 native tests/13suites and four tiny modes, including actual13scheduled/11useful/2unused counters and unchanged routes/cache/logits. Newbinary full-predicted-read.exe, candidatewrapper run-full-prefetch.ps1; production wrapper staysb0 untilqualified. Wait for107 complete and no full process before building.107 controller34086 running.

107 initial attempt failed in SSH banner exchange before launch; raw output empty. Native recheck proves no full process and no107-heldout-prediction files. Preserve failure/history and launch identical107b retry, keeping107 native artifact names. No model/correctness gate bypass.

SSH timeout diagnosis: Autolab SSHRunner defaults connect_timeout10 and appends ssh_options afterward. Thus extra ConnectTimeout30 did not override the first option; local ssh -G confirms10. Use runner.connect_timeout:30 in future campaigns. Existing/running configs remain truthful.107b retry active native4424(created1789383292.9362197), controller12159. No modification to pre-existing dirty siblingAutolab ssh.py was needed. db496530 push retry succeeded.

109 source dc4badd3 staged with six source files/eleven frozen binaries and all candidate SHA checks; PS parser passes. Candidatewrapper3eac49ad3042d50a4425f372e24fed12ff9b8b5b983974dfe0d3ee4d3262a7a7. Detachedparent3856 launched waiter after107; no build during full run.110/111/conditional112 campaigns prepared with actual connect_timeout30 (SSHRunner arguments verified). Causal counter expectations11111906/11338/568 and11235718/34014/1704 scheduled/useful/unused; repeated forecast assumes deterministic reset inputs and must match actual counts.

107b completes throughAutolab and all107 native artifacts areSHA captured. Unseen first-uncached prediction precision94.814%,27.605% miss coverage,1.510% extra reads (24124scheduled/22873useful/1251unused). All128IDs/fullhash/actualroutes exact, actualcache reads down4.836%. Diagnostic throughputalgebra.90215vsreference.94204,JSON.90125vs.94585,dialogue.885139vs.885341; no speed claim. NoMLPevent>.5s, so no101-style stall to exclude. Attention+MLPphase comparisons saved107_heldout_comparison.json. Native109 building and testing, PID8624/launcher9300; new local controller will archive109, sync wrapper afterqualification andrun110/111 sequentially.

109 native qualified252tests and four fulltiny modes. Controller58085 copied andSHA verified all artifacts except final109-fixture-recent1-read1-predicted.json, whose SCP failed at jump-host kex. No110 launched and localwrapperstillb0; nativewrapperalreadyqualified3e. Resume controller skips only alreadySHAverified files and retries missingtransfer, preserving any partial copies. This is transport recovery, not a qualification failure.

109 archive transport recovery succeeded; all artifactSHA checks pass, native/localwrapper both3eac49ad. Nativefull-predicted-read.exe7b20ee3592cb09267787792f2994a7cb54555f1dc0f1a74bf2d3517b0ead6f7a/source dc4badd3. Controller92370 starts110control native7308(created1789384238.1766171), then111prefetch. Conditional112 staysprepared.107matched identical read-set diagnostic14242visits showsmedianMLP+1.88% andattention+5.55%; run-time variation/observer effect not causally isolated. No107samples discarded.

110 freshcontrol complete.9679409165tok/s, allnumeric/config/cache/I/O/predictionzero gatespass; rawartifacts/SHA captured.111 predicted-readcandidate running3408(created1789384539.2968717), controller92370. No promotion yet;112 conditional.

111 fullprefetch candidate complete:water1.0607746934,binary1.1050900775,story1.1374304863, gains6.33/14.17/14.64% vsfresh110. Conservative gain9.5909%. Exact outputs/routes/cache counters and actual11906scheduled/11338useful/568unused, zero failures; extra18.091GB reads1.4446%. Rawartifacts/SHA verified;111_prefetch_comparison.json. Start112 nine-sample confirmation before record/profile promotion.

113/114 prepared to isolate actualprefetch benefit on the frozen unseen128-token prompts after112. Both use same qualifiedfull-predicted-read.exe, recentcache1 and no prediction-trace diagnostic; only PredictReads differs. Preserve exactheldout hash/IDs/routes/cache counters.114forecast24124scheduled/22873useful/1251unused from107 observedpredictions. These are separate fromcanonicalrecord. This resolves the earlier106/107 comparison confound (cachepolicy and observer both changed).

115 hypothesis: applying the target layer's existing MLP norm/router to the residual entering its predecessor may predict useful experts early enough to overlap a complete preceding layer. Add a default-off, decode-only observer at Model scope, before the predecessor executes. Do not perform new I/O, alter actual routes/cache history, or reuse future activations. Trace labels explicitly distinguish this one-layer-early input from current-layer pre-attention prediction. Local/native exact-logit and causality tests precede any full-model diagnostic; accuracy and read amplification decide whether a runtime experiment is justified. Native builds wait until112/113/114 finish.

112 repeated confirmation passes all nine exact outputs/routes/cache/predicted-read counters. Slowest1.0904267748, median1.1492074013,max1.2299078616; +12.9494% vs102 and8.0573xoriginal. Allnine improve9.53–16.18%. Firstwater second-token layer48 MLP stall2.6689s remainsincluded. Actual35,718successful/34,014useful/1,704unused,0failures,1.4456% extrareads. MinavailableRAM1.85GB,peakprivate42.52GB. PromotePredictReads1 and SHA syncprofileff9572f5.113control now8876(created1789385817.5668454),controller73146 queues114.115 local250tests/13suites+example pass; fixture/analyzer qualification pending. No nativebuild during113/114.

115 local qualification complete:250tests/13suites, four fixture modes off/current/previous/currentprefetch preserve every output/actual route/cache and existing predicted-read count. Analyzer validates declared prediction source/lead, rejects missing positions/changed hash/false prefetch claims. Tiny one-layer-early16predicted/10useful/6unused; no full-model accuracy inference.116 candidatewrapper/nativequalifier prepared to preserve12older binaries and require254tests/fourfixture modes after113/114.

116 source23412241 staged with3Rust files,12frozenbinary identities, SHA archive8f1d4ecc and candidatewrapper6db0fff9. Native PowerShell parser passes. Detachedwaiter4748(created1789386222.7887948)/parent10316 waits113/114. Controller11957 will archivequalified254tests/fourfixture modes and launch117 diagnostic after114 correctness comparison.113completed0.9165585148;114native2540(created1789386354.6752503). Originalfeat/inkling worktree rechecked11:45UTC clean/unchanged9aaebff0.

113/114 longer same-binary comparison completes, all128IDs/fullhash/routes/cache and realread counters exact. Threeprompt gains15.02/16.15/15.32%; conservative0.9165585->1.0570072 (+15.3235%). Actual24124scheduled/successful,22873useful,1251unused (39.845GBextra),allfailures0. NoMLPevent>.5s in eitherarm; firstsamplesincluded. MinavailableRAM1141.69GB. Confirms benefit on previously frozen unseenprompts; separate fromcanonical112record. Controller73146/native2540 exited;116 nowbuilding/testing,11957 waits then117.

116 qualified254native tests/fourfixture modes; newfull-early-prediction.exe853c4cbe/source23412241. All12oldbinaries and existingruntime outputs/routes/cache/prefetchcounters preserved. Controller11957 transferfailed onlyfinal116tiny predictionfile afterthree jump-host resets; allpreviousfiles SHA valid. Resume18569 enables dedicatedControlMaster/ControlPersist60, skipsverifiedcopies; transfer succeeds. Allrawartifacts archived, native/localwrapper6db0fff9 synced.117 now10960(created1789387032.1257544), source23412241, threeoriginalprompts×1rep, noactualprefetch, diagnostic-only.

118 hypothesis: repeatedly opening uncached expert files may add measurable latency. Before any runtime handle cache, run30 balanced matched read-only pairs (2/4/6files) comparing fresh open/read/close against exclusive preopened handles with a timed seek-to-zero, the same two size checks and complete aligned read. Preopen cost is recorded separately, not hidden; exclude fullbenchmark route files, hash both arms against a mapped oracle aftertiming, retain allpairs/order effects. Probe waits117 exit and holdsbaseline lock. It measures a component upper bound, not full-model speed. Primary APIs checked: https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-setfilepointerex and https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-getfilesizeex ; handles are exclusive to one concurrent operation, offsets sectoraligned.

119 runtime hypothesis based on117: one-layer-earlier first-uncached prediction has86.9602% precision,26.4249% misscoverage,3.9625% extra reads (11948scheduledforecast/10390useful/1558unused). Test whether the longer overlap window outweighs reduced accuracy, comparing current-layerPredictReads1 against earlyPredictReads1 in one qualifiedbinary. Default-off earlyflag, at most current+next pending expert per model; immutable full buffers, unchanged actual routes/cache admission, and drain/fallback semantics remain. Firstlayer has no predecessor and keeps its current-layer behavior. A two-slot request channel prevents submission from blocking behind the current request; the one worker and per-request completion ownership remain. Add a blocked-worker queued-request test and qualify tiny exact outputs/routes/cache/read counts before native full A/B. No runtime early-prefetch code is implemented yet.

117 complete/SHAverified, all outputs/routes/cache/read counters exact, diagnosticrate.9468468067. Firstuncached one-layer-early11948predicted/10390useful/1558extra,86.9602% precision/3.9625%extra.119 runtime experiment justified by longeroverlapwindow, subjecttoactualfullA/B.118 read-onlyprobe completeandSHAcaptured:30pairs,ratios2/4/6files.99942/1.00785/1.01242,order-sensitive; no reliablewin, rejecthandlecache. Canaryremoved/allhandlesclosed/protectedservicespreserved. No activefullprocess; start119localimplementation.

119 local251tests/13suites andfive completefixture modes pass. Currentprefetch13/11/2 andearly16/10/6 scheduled/useful/unused exactly match earlierdiagnostics; all outputs/hash/routes/cache counters unchanged. EarlyflagwithoutPredictReads performsno I/O. Newblockedworker test verifies two queuedrequests progress/ownership, existing failure/unwind tests pass.120 nativewrapper/qualifier prepared for255tests/fourmodes andinvaliddependency rejection.121current/122early/conditional123repeated campaigns prepared; repeatedforecast replays allnine102 routes/cache state using deterministic117predictions ratherthanblind multiplication.

120 sourceb53a984b qualified255native tests/fourfixture modes+invaliddependencyguard. All13olderbinaries unchanged; allnativeartifactsSHAcopied with connectionreuse. Newfull-early-prefetch.exe381c1166, productionwrappernative/localca787020. SelectedprofileexplicitEarly0guard synced5f7a0440; confirmed112recordunchanged.121current now9348(created1789388242.4688969),controller4645 queues122early.123conditionalprepared with actualnine-cache replayforecast35844scheduled/31168useful/4676unused, including2extraunused vsnaive singlepass multiplication.

124 analysis hypothesis: a second concurrent current-layer prefetch may be useful only when its predicted rank is high enough to avoid the5.57% extra reads of unconditional two-prediction prefetch. Replay105 calibration predictions against prelookup cache state, choosing firstuncached plus a second only under fixed rank ceilings. Choose any candidate policy from105 before checking frozen107 longer-prompt predictions. Selection must not use futureactual routes. No runtime code or speculative speed claim yet.

121/122 completes exact outputs/routes/cache/predicted-read counts, but earlyprefetch slows all3prompts by5.37/1.52/1.42%, score1.104303->1.065937(-3.47%). Watersecond-token MLP layer52 stall2.4918s remainsincluded; otherprompts also regress. Rejectearly, keepselectedEarly0;123notlaunched.124causal rankanalysis selectedsecondpredictedrank<=2 (topthree) from105calibration under1% additionalreadbudget, then froze policy before107longer check. Calibration adds5511useful/303unused (.7596% extra overfirst-only); longer adds11625useful/829unused(.9856%). Totalmisscoverage42.85%/41.63%, precision95.08%/94.31%. Runtimegainnotmeasured.

125 next hypothesis: concurrently prefetch a second current-layer expert only when its predicted rank is amongtopthree and it is the seconduncached prediction. Keepfirstuncached policy unchanged. Use a separate bounded second reader so both may overlap attention, atmosttwo pendingbuffers perlayer. Mutuallyexclude EarlyPredictReads and SecondPredictReads inwrapper; library earlymode takesprecedence. Preserve original9counter stats as total; expose secondworker actual counters separately. Add nonmutating cache-selection and independent-worker/buffer-identity tests, then qualifyexacttiny/full A/B before promotion. No125runtimecode yet.

125 implementation and local qualification complete: bounded first/second independent readers, original rank ceiling2, unchanged default first-only path, early flag takes precedence in library and wrapper rejects combined modes. All387 tests across60 reported sections pass (full local crate suite), plus eight full tiny-model modes including disabled dependency and combined flags. Exact IDs/hash/routes/cache counters; second-enabled forecast and actual19scheduled/16useful/3unused, of which second6/5/1, zero failures, two workers. Native125 fixture forecast agrees independently. Stage126 native257-test/13suite qualification preserving14 frozen binaries, five fixture modes and three invalid dependency guards. Then127/128 matched single-pass A/B;129 only if gain warrants repeated confirmation. No speed claim or profile promotion.

126 native build/testing active qualifier9852(created1789389869.1730728), launcher10892; sourceede10f98, six source replacements,14 preserved binaries, sourcearchiveSHA db3e4521, wrapper09f20b18 parser/SHA verified. Local controller will archive126 then127/128. Corrected128 byte forecast before launch using measured expert size31,850,496 bytes. Conditional129 forecast is an actual nine-sample cache replay using deterministic105 predictions in112 sample order:53152scheduled/50539useful/2613unused, second17434/16525/909; not naive3x single-pass. All first-reader counts match112. No new exports required.

126 native qualification complete:257tests/13suites, five exact fixture modes, three rejected invalid combinations;14 old binaries preserved. Newbinary71f7e4ef/sourceede10f98, wrapper09f20b18. All22 proof artifacts SHA captured.127 control active2584(created1789389969.340804), then128 second candidate via controller. Selected profile explicitSecond0 SHA4b076b91; only future-shell guard, no active configuration changes.129 remains conditional.

127/128 complete: matched current score1.1036037599 vs second1.1181528314, conservative+1.3183%. Allthree+1.4921/1.3183/1.2417%, exact IDs/fullhash/routes/cache counters, forecast17720scheduled/16849useful/871unused, second5814/5511/303, zero failures. Additionalreads0.759646% over first-only. Medianattention.21583→.21167s,MLP.64064→.63307s,outside.02447→.02444s; noMLPevent>.5s eitherarm. Candidate minavailableRAM1.879GB,maxprivate42.500GB. All raw/SHA/resources archived. Launch129 nine-sample confirmation before profile promotion; confirmedrecord112 retained.

130 next hypothesis (before code changes): selective second prefetch127/128 gains only1.2–1.5% despite raising covered misses by14 percentage points. Test whether a tighter or looser original predicted-rank ceiling better balances overlap and wasted reads, keeping exactly two bounded workers and unchanged first prediction. Add experimental rank1..5 with default2; reject invalid wrapper dependencies. Local/native exact qualification and frozen causal forecasts precede132+ full sweeps. Native131 build must wait for129 and its verification to finish; productionwrapper/profile remain unchanged during129. No performance claims or retuning on running129 outcomes.

130 local qualification complete:255tests/13suites, ten exact full tiny modes including rank1/2/5, disabled dependency and early precedence. Original rank boundary/duplicate/cached membership and invalid rank parsing tested. Productionwrapper/profile remain09f20b18/4b076b91 during129; only candidate run-full-second-rank.ps1 changed.132rank2control,133rank1,134rank3,135rank5 campaigns prepared with previously frozen124 forecasts and31,850,496-byte expert accounting. Native131 qualification/build waits for129 complete and SHA verification. No runtime gain yet measured for alternate ranks.

129 verified/promoted:1.1161344306 min9, median1.1515257935,max1.2312886560,8.2473×original,+2.3576% vs112 score. Exact everyID/hash/route/cache/readcounter;53152scheduled/50539useful/2613unused, second17434/16525/909, zero failures. Two samples are within.02% below112, median+.2017%; historical firstwater2.67s stall remains inoldrecord. Matched127/128 gains1.2–1.5%; do not attribute entirehistorical2.36% to this setting. No129MLPevent>.5s. MinavailableRAM1.837GB,maxprivate42.505GB. ProfilePredict1/Early0/Second1/Rank2 promoted local/nativeSHA722f4758. Guarded45173 proceeded after129verification+comparison, staged131 parent7408 andnativebuild. Source6c40e442,15 frozen binaries, then132–135 rank sweep.25target unmet.

131 native qualification complete:259tests/13suites, seven exact tiny modes andfour invalid dependency guards,15 older binaries preserved. Newfull-second-rank.exe SHA83a1a50f49efff8b973bad6f2a89464f30ce47521a9ecbaf086b3c3a9a1b1ee2/source6c40e442; wrapperbe0b9fb0 local/native. All qualification artifactsSHA archived.132rank2 control native4248(created1789391471.5424938) active; controller45173 queues133rank1,134rank3,135rank5.129record/profile remains selected;25target unmet.

136 independent diagnostic hypothesis, before replay: combine one earlier prediction with a current-layer correction. If the earlier first-uncached expert agrees with the current first-uncached expert, optionally add the current second prediction up to a fixed rank ceiling; if they disagree, add the current first. This may give the additional useful read more lead time. Replay only current/predecessor predictions and prelookup membership, then score against actual routes. No runtime implementation or speed claim. A future implementation would require a separately qualified bound of up to three live requests (two current plus one next), still two reader workers; existing early+second flags remain mutually exclusive. Reject if forecast coverage/read cost gives no useful reason to test.

136 causal hybrid forecast complete, no new runtime I/O: early+current agreement8906/12096 visits. Hybridcap2 schedules19443/17358useful/2085unused,44.15% miss coverage and5.30% extra reads over actual misses. Preparedcurrentcap3 instead covers47.67% with18742useful/1215unused (3.09% extra). Hybridcap5 also covers fewer misses with more unused reads than currentcap5. Defer hybrid runtime: poorer read-set tradeoff and prior122 early regression; earlier completion might still help, so this is prioritization rather than a proven speed result.136_hybrid_prediction_forecast.json/136_hybrid_decision.json.132–135 remain the active measured sweep.

138 forward-looking forecast hypothesis (no runtime edit): a third read is considered only when all three leading predicted experts are uncached, fixing its original rank ceiling to2 before replay. Preserve the existing first and selected second policy. Require at least3 percentage points additional miss coverage and no more than1% additional completed read bytes over the two-reader baseline on both105 and107 before prioritizing runtime work.107 is now a reused longer-prompt check, not a newly unseen validation set. A future implementation would need separately qualified third-worker ownership, memory bounds and exact full outputs. Finish the active rank sweep and any137 confirmation first.

138 forecast meets its predefined budget on both datasets. Third rank2 adds2058reads/1770useful/288unused in105, raising miss coverage4.50 points with.7166% extra reads over current rank2two-reader.107 reusedlonger check adds4200/3669/531,4.43coveragepoints and.6252% extra reads. No third runtime exists; prioritize only after rank sweep and any137confirmation. Qualify third-worker ownership/fallback/draining and scratch memory bounds before full trials.133rank1 completed1.1078773695, slower than132rank2control1.1182391858; alloutputs/routes/cache/countergatespass.134rank3 active9888(created1789392029.1779904), then135rank5.

135 rank sweep complete, retain2.132control1.1182391858;133rank1=1.1078773695,134rank3=1.1103680145,135rank5=1.1033624071. Every alternative loses allthree cases, all exact outputs/routes/cache/readforecasts, noMLPevent>.5s. Skip137confirmation. Peak sampleddecodeI/O approaches7.97GB/s, but this is not proof of SSDmaximum; actualPCIe5x4 link property remains15.754GB/s theoretical. Prior069 already tested1/4/8MiB chunks and073 pairedwholeasync had no consistent gain; no redundant chunkprobe.

139 runtime hypothesis, before code: add a third independent bounded current-layer read only when all three leading predicted experts are uncached (thirdrank2). Keep selectedsecondrank2, at mostthree pending requests, mutually exclusive with early scheduling. Raise idle scratchpool cap only with requestedthird mode from256 to320MiB to avoid recurring extra allocation; include this cost/config in full A/B. All aggregate counters retain9fields and add separate third-worker counters. Qualify complete ownership/identity/fallback/draining plus originalHFtop2 fixture and a top3 manifest variant referenced by the frozen prior binary. The variant changes only tiny test routing, not the real export. Full matched trials only after nativequalification.

139 local implementation qualified:257tests/13suites,18 full tiny modes, plus focused3-reader unwind-guard test. OriginalHFtop2 hash5122e042 remains exact; separately copiedtop3manifest changes no weight bytes and is referenced against frozen pre-third binary (Macceec21ab, native83a1a50f). Mac top3hashcab94449a78517d3/native3b872132795009f1, same8 IDs. Top3off actual20misses, two-reader17useful and third20useful including3actualthirdreads; allroutes/cache/bytes exact, zero failures. Bothindependentforecasts predict3/3/0thirdreads.Third requiresbasePredict+Second+nonzero cache and suppresses underEarly; idlepool256→320MiB onlyrequestedactiveconfig, oldprofileunchanged. Full143 replay predicts59288scheduled/55811useful/3477unused, third6136/5272/864; not naive3×single-pass.140 native qualifier prepared261tests,11fixturemodes,6invalidguards, preserving16frozenbinaries.141two-control/142third-candidate andconditional143 prepared. No fullthird speed measured.

140 staged sourceaebab266, sixsourcefiles/16preservedbinaries, archive45d4e219/candidatewrapperc18b8446; nativePSparser andSHAchecks pass. Qualifier1920(created1789393819.9274683), launcher1676. Controller/private/tmp/run-inkling-third-prefetch-loop.py archives140 then141two-control/142third-candidate, stops forreviewbeforeconditional143; preparedverify/compare/run143 helpers. Nativeprotectedservices unchanged.

144 hypothesis before replay: fixed8slots perlayer can strand capacity in weakly reused layers. Compare a causal model-wide512-entry LFU cache with current per-layer8, retaining ceil decay32/recent ties. Candidate frequencies decay on each layer own32-observation clock, ties use global past-access timestamps. Reset history between requests and preserve entries, observe the entire actual cohort before gate-order admission. Replay rank2 two-reader preselection against prelookup membership to include speculative read costs. Require at least2% fewer total completed reads on105 and reused107, with no case regression, before native implementation priority. Forecast only, fixed total retained bytes.

140 qualification passed261tests/13suites,11fixture modes,6invalid guards;16prior binaries preserved. Newbinaryf91e4e0e/sourceaebab266.141/142 exact outputs/routes/cache/readforecasts;third loses allthree by0.592/0.330/0.543%, conservative1.115971895→1.112292152. Rejectthird/skip143;keep129record1.116134431/explicitThird0. Profile staging template incorrectly substituted SHA inside OWN_SHARED, caught by post-write hash; no full process launched. Repaired via base64 and SHA verified local/native. All samples included, no export/Lambda.

144 shared-cache forecast rejects candidate: completed reads increase3.737% on105 and4.187% onreused107, everycase worse. Independent perlayer control matches actualruntime and separateadmission/readpredictor counters. No shared runtime implemented. Reassessment after rank/third/cache losses: leave cache and prefetch policy fixed; attention still costs~.212seconds/token and Rayon scheduling now overlaps two independent prefetch readers, a condition absent064–068.145–148 hypothesis: reducing compute pool16→14 or12 may reduce contention without lowering read concurrency. Qualify allthree with frozenbinaryf91e4e0e and currentwrapperc18, then fresh16control,14candidate,12candidate in sequence. All16CPUs remain eligible; no process affinity or global changes. Require alloutputs/routes/cache/readcounters exact, allsamplesincluded. Repeat a winning candidate before promotion; otherwise retain16. This is a targeted retest after scheduling changed, not a claim that the earlier sweep was invalid.

145native qualification passed16/14/12workers, exact originalHFtiny tokens/fullhash/routes and cache/readcounters, allfrozenbinary/wrapper/profile hashes unchanged.146fresh16worker control launched native10736(created1789395814.2578628);14714worker/14812worker queued. All16CPUs remain eligible; same readconcurrency.

150 independent code hypothesis, before implementation: BF16 GEMV dispatch currently lets Rayon subdivide down to one2-rowtile. A bounded minimum scheduling grain of16/32/64output rows may reduce task scheduling and improve locality while preserving each row's arithmetic and all weights. Add opt-in default1minimum-row setting for the existing tiledAVX2 path only, with default behavior unchanged. Existing row arithmetic tests and complete tinyfixtures must pass before a separately frozen native build. Do not stage/build or measure it onPTL until146–148 and any149confirmation finish. Full speed claims require a samebinary control/candidate and repeated confirmation.

150local implementation: opt-inCASCADIA_BF16_GEMV_MIN_ROWS1/16/32/64, default1; tiledAVX2BF16rows2/4 only. Rayonminimum is ceiling(minrows/tile), row arithmetic/weightlayout unchanged. Expanded existing bit-exact row-tail test to65/129rows across grain boundaries.257localtests/13suites and9tinyconfigurations pass, including invalidenvfallback and preservedtop3readerpath. ARM cannot executeAVX2, so native151qualification remainsrequired. Prepared candidatewrapper/batch/qualifier;264native testpasses/16sections expected(261main+3focusedgrainprocesses),17fixturemodes and8invaliddependencyguards. Not yetstaged;146–148active.

151source manifest/archive prepared from735e4ca9, onlymath.rs+decodeexample changed;17oldbinaries preserved. Native candidatewrapperc172f391 and tests/qualifier prepared. Stager requires148decision ready_for_next_runtime=true; notlaunched.152–155grain1/16/32/64fullcampaigns prepared atselected16workers; adjustonlyif149confirms anotherpool.146control watersecond-step has3.02755sMLPlayer60 and2.480884sMLPlayer62 stalls; allincluded. This makes scoregain alone insufficient to attribute workerbenefit.

14714workers completed1.090794268, waterincludes2.433sMLPlayer34 atseconddecodestep; binary1.110525vs146control1.118256 so notallcasesbetter.14812workers active3948created1789396379.6980217.147_stall_cache_membership replayexactlymatchesactualcachecounters: all3long146/147events occur with6/8cacheentriesbeforelookup, while cachecanstillgrow. This is correlation; allocation/read timing needed for attribution. Coordinatorcontinue-inkling-after-workers.py waits148, conditionallyretains16andstages151orreturnsfor149review; no duplicates.

148sweep finished, retain16/skip149.14losesbinary.691%/story.834%;12losesbinary1.344%/story1.616%. Controlwaterstalls remainincluded; neitherpoolpassesallcasegain, scoresbelowconfirmed129. Alloutputs/routes/cache/readforecasts exact; rawartifactsSHAcaptured. Coordinator3027 nowstages151 andqueues152–155grainexperiments, fixed16workers. OtherInklingworktree recheckedcleanunchanged9aaebff0.
